...
function loadConfig() {
  const envVal = process.env.GOOGLE_APP_CONFIG_JSON;
  if (envVal && envVal.trim()) {
    try {
      return JSON.parse(envVal);
    } catch (e) {
      throw new Error(`GOOGLE_APP_CONFIG_JSON env var is geen geldige JSON: ${e.message}`);
    }
  }
  const p = configPath();
  if (!fs.existsSync(p)) {
    throw new Error(
      `Geen config gevonden. Stel env var GOOGLE_APP_CONFIG_JSON in (productie) of plaats ${p} (lokaal). ` +
      `Zie config/google.example.json voor het formaat.`
    );
  }
...
function loadCredentials() {
  ...
  if (envVal && envVal.trim()) {
    try {
      ...
      throw new Error(`GOOGLE_SERVICE_ACCOUNT_JSON env var is geen geldige JSON: ${e.message}`);
    }
  }
  ...
  if (!fs.existsSync(keyFileAbs)) {
    throw new Error(
      `Geen service-account credentials gevonden. Stel env var GOOGLE_SERVICE_ACCOUNT_JSON in (productie) of plaats het ` +
      `service-account JSON-bestand op ${keyFileAbs} (lokaal).`
    );
  }
